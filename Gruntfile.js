...
const angular = comprehension(
    {
        ...
        'ng-build': 'ng run contacts:build:prod '.concat(buildArgs),
        'ng-ssr': 'ng run contacts:server:prod',
        'ng-unit': 'ng test '.concat(testArgs),
        'ng-integration': 'ng run contacts:integration '.concat(testArgs),
        'ng-e2e': 'ng e2e',
    },
    ([k, v]) => [k, { cmd: v, cwd: 'contacts' }]
);
...
const images = {
    ...
    contacts_nginx: { ctx: 'entry' },
    contacts_server: { ctx: 'contacts_server' },
    contacts_server_db: { ctx: 'postgres' },
};
...
module.exports = function (grunt) {
    ...
    grunt.registerTask('serve', 'Serve the contacts locally', 'exec:ng-serve');

    grunt.registerTask(
        'runserver',
        'Run contacts_server locally',
        'exec:runserver'
    );

    grunt.registerTask(
        'build',
        'Build contacts for the browser with production settings',
        'exec:ng-build'
    );

    grunt.registerTask(
        'ssr',
        'Run contacts for ssr with production settings',
        'exec:ng-ssr'
    );
